...
import { requestOptions, requestPause, requestPlay, requestPlayNext, requestRestart, requestVolume } from 'store/modules/status'
...
const PlaybackCtrl = props => {
  ...
  const handleRestart = useCallback(() => dispatch(requestRestart()), [dispatch])
  ...
  return (
    <div className={styles.container}>
      ...
      {status.isAtQueueEnd &&
        <div onClick={handleRestart} className={styles.restart} title="Restart Queue">
          <Icon icon='REPLAY' size={44}/>
        </div>
      }
